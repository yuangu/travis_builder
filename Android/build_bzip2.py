...
install(TARGETS bz2
    ...
    '''
    with open(os.path.join(srcPath, "CMakeLists.txt"), "w+") as f:
        f.write(CMakeLists)
...
def do_build(config, installPath):
    ...
    downloadUrl =  'https://www.sourceware.org/pub/bzip2/v102/bzip2-%s.tar.gz'%(config['version'],)
    ...
    Utils.cleanFile("./bsdiff")
    Utils.mkDir("./bsdiff")
    Utils.extractTarFile(outZipPath, "./bsdiff")
   
    #解压完毕,进入源代码目录
    mbedtls_dir_list = os.listdir('./bsdiff')

    #获取源代码
    cwd = os.getcwd()
    srcPath = os.path.join(cwd,'./bsdiff', mbedtls_dir_list[0])
    os.chdir(srcPath)

    #生成cmake
    createCmakeFiles(srcPath)
